...
function payload(expires = '2027-12-31T00:00:00.000Z'): string {
  return [
    ...
    'Canonical: https://ffcworkingsite1.org/FFC-EX-nu4children.org/.well-known/security.txt',
    'Canonical: https://ffcworkingsite1.org/FFC-EX-nu4children.org/security.txt',
    'Policy: https://ffcworkingsite1.org/FFC-EX-nu4children.org/vulnerability-disclosure-policy',
    'Acknowledgments: https://ffcworkingsite1.org/FFC-EX-nu4children.org/security-acknowledgements',
    '',
  ].join('\n')
}
...
describe('security drift guard', () => {
  ...
  it('requires root lines (not project-path lines) once public/CNAME exists', () => {
    ...
    const dir = makeFixture({ cname: 'nu4children.org' })
    fixtures.push(dir)
  ...
  it("treats a whitespace-only public/CNAME as configured, matching deploy.yml's `[ -s ... ]` check", () => {
    ...
    expect(result.status).not.toBe(0)
    // Missing the now-required root lines, same as the real-domain case above.
    expect(result.output).toContain(
      'Missing: Canonical: https://ffcworkingsite1.org/.well-known/security.txt'
    )
    expect(result.output).toContain('misdirects to')
  ...
  it('treats an unreadable/non-file public/CNAME as configured too, alongside its own read-error report', () => {
    ...
    // Still evaluated as CNAME-configured: root lines required, project-path
    // lines from the default fixture payload now misdirect.
    expect(result.output).toContain(
      'Missing: Canonical: https://ffcworkingsite1.org/.well-known/security.txt'
    )
    expect(result.output).toContain('misdirects to')
